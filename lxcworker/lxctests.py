...
def getContainerIP(container_name):
	container=lxc.Container(container_name)
	for container_ in lxc.list_containers():
		if container_==container_name:
			container = lxc.Container(container_)
...
if len(sys.argv) <2 :
	print ("please specify a command from [listRulesOnDPort, deleteRulesOnDPort, addRedirect,addRedirectToContainer, deleteRulesForSourceIPi, deleteRulesForDestIP]")
	exit()
else:
